...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        // IMPORTANTE: Aca verifico lo que viene por req.quey  
        //console.log("req.query",req.query)

        // *** Nota: El 4to Parametro SOLOCITADO en la Pre-entrega 'Query'
        // lo uso para definir el [query]criteria que me suministra "mongoosePaginate"
        let { page, limit, sort , query } = req.query

        // Paso 3: Usando el Metodo .getProducts() disponible en archivo product.controller.mdb.js
        const products = await controller.getProducts(limit, page, sort, query)

        // Aca Mandamos la respuesta al cliente con el listado de productos encontrados 
        res.status(200).send({ status: 'success. Productos-Paginados', payload: products })
    ...
    //----- Rutas para hacer la consulta del Lado del cliente -----------

    // Ruta con todo --->    http://localhost:5000/api/products/?page=2&limit=5&sort=desc&query=fruta
    // sin page y query ---> http://localhost:5000/api/products/?limit=5&sort=desc
